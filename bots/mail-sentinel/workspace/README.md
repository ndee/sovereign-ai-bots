...
- `config/default-rules.json` contains the packaged baseline local relevance rules and is refreshed from the bot package on install/update to keep shipped heuristics aligned with the current bot version.
- `config/user-policy.json` is the operator-managed override layer; operator-specific adjustments belong there and in instance state, separate from the packaged defaults.
...
- visible AMBER digests number the rendered items but still hide alert IDs; when an operator replies to a digest item, Mail Sentinel resolves the target by subject or sender.
