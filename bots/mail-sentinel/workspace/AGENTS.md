...
5. For “Why was this flagged?” / “Explain this alert” / debugging a wrong classification, use `explain` (`--alert-id`, `--latest`, or `--ref`); it reports the matched rules and policy modifiers, the semantic reviewer's verdict, and the final zone decision as separate sections
6. For direct sender importance requests like “Mails from Nadine are important”, use `policy important-sender --query <text> --announce` first
7. For other sender/domain preference changes, use `policy list`, `policy add`, and `policy remove`
