...
describe("catalog validator", () => {
  ...
  it("passes lint, typecheck, test, and smoke for the current catalog", async () => {
    ...
    await expect(lintCatalog(repoRoot)).resolves.toMatchObject({ errors: [], jsonFileCount: 13 });
  ...
  it("runs the catalog CLI and reports usage errors", async () => {
    ...
    expect(lines).toEqual(["Lint passed for 13 JSON files."]);
  ...
  it("covers helper branches and failing command output", async () => {
    ...
    expect(commandLines.some((line) => line.startsWith("Smoked mail-sentinel@2.0.10-test.1"))).toBe(
      true,
    );
